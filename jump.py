...
medfont = pygame.font.Font("COOPBL.TTF" , 40)
...
c.execute("CREATE TABLE IF NOT EXISTS high_score(SCORE INTEGER PRIMARY KEY)")
conn.commit()
print("Table created")

def bst():
     c.execute("SELECT MAX(SCORE) FROM high_score")
     bt = c.fetchone()[0]
     print("bt = ",bt)
     ...
     if bt == None:
                  ...
                  c.execute("SELECT MAX(SCORE) FROM high_score")
                  best = c.fetchone()[0]
     print("best in bst= ",best)
     return best

def sql(points):
     c.execute("REPLACE INTO high_score( SCORE ) VALUES (?) ",( points , ) )
     conn.commit()
     
     c.execute("SELECT MAX(SCORE)  FROM high_score")
     best = c.fetchone()
     print("best in sql =" ,best[0])
     conn.commit()
...
class MainGuy (pygame.sprite.Sprite):
    ...
    def slide(self):
        self.image = pygame.image.load("player_slide.png")
        self.image = self.image.convert_alpha()
...
def main():
    ...
    while keepGoing:
        ...
        text_1= medfont.render("BEST : " + str(best) , True , black)
        screen.blit( text_1 , [300,2])
